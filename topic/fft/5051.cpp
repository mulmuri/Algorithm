...
int main() {
    ...
    map<ll, int> root;
    for (ll i=0; i<n; i++) root[i*i] = i;

    vector<ll> n1(n);
    for (ll i=1; i<n; i++) n1[(i*i)%n]++;

    vector<ll> n2 = multiply(n1, n1);
    for (int i=0; i<n; i++) n2[i] += n2[i+n];
    for (ll i=1; i<=n; i++) n2[i*i*2%n]++;
    for (int i=0; i<n; i++) n2[i] /= 2;
